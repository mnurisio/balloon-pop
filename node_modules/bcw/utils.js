const { colors, printSuccess, printAlert, printWarning, printInfo } = require("./utils/console-utils");
...
function configureGit(name, ghemail) {
  ...
  execSync(`git config --global pull.merge true`);
  execSync(`git config --global init.defaultBranch main`);
  printSuccess(`[+] Git configured: [${name} | ${ghemail}]`);
  console.groupEnd();
}

async function getGithubUser(username) {
  console.group(colors.FgMagenta, "---VALIDATING GITHUB ACCOUNT---", colors.Reset);
  const ghAccount = await get(`https://api.github.com/users/${username}`)
...
async function verifyGithubRepo(username, projectName) {
  ...
  const repo = await get(`https://api.github.com/repos/${username}/${projectName}`)
  if (!repo || !repo.name) {
    throw new Error("Error verifying github repository")
  }
  printSuccess(`[+] Github Repo Found: [${repo.name}]`)
  return repo
}
